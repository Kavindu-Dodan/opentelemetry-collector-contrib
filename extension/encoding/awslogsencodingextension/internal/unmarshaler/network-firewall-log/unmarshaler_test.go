...
func TestNewLogsDecoder(t *testing.T) {
	...
	expectPattern := "alert_log_multi_%d.yaml"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data, err := os.ReadFile(filepath.Join(directory, "alert_log_multi.json"))
			require.NoError(t, err)
			reader := compressToGZIPReader(t, data)
			...
			streamer, err := networkUnmarshal.NewLogsDecoder(reader, encoding.WithFlushItems(1), encoding.WithOffset(tt.offset))
			require.NoError(t, err)
